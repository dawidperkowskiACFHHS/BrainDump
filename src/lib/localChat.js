export function answerLocalQuestion(question, user) {
  ...
  // List documents
  if (q.match(/what (files|documents)|list (files|documents)|show (files|documents)|how many (files|documents)/)) {
  ...
  if (searchMatch) {
    ...
    const results = user.documents.filter(d => 
      d.content.toLowerCase().includes(term) || d.filename.toLowerCase().includes(term)
    );
